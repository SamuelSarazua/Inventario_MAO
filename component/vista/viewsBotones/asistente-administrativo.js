...
function administrativo() {
    const datosGuardados = localStorage.getItem('asistenteAdministrativoEditados');
    ...
    if (usuario && (usuario.rol === 'editor' || usuario.rol === 'admin')) {
        ...
        function guardarCambios() {
            ...
            localStorage.setItem('asistenteAdministrativoEditados', JSON.stringify(nuevosDatos));
            return nuevosDatos;
        }
        ...
        if (usuario.rol === 'admin') {
            ...
            botonAgregar.addEventListener('click', () => {
                ...
                [1, "Nuevo Producto", "/", "ASISTENTE ADMINISTRATITVO", ""].forEach(text => {
                    const td = document.createElement('td');
                    td.textContent = text;
            ...
            botonRestaurar.addEventListener('click', () => {
                if (confirm('¿Restaurar datos originales?')) {
                    localStorage.removeItem('asistenteAdministrativoEditados');
                    document.getElementById('root').innerHTML = "";
                    document.getElementById('root').appendChild(administrativo());
                }
            });
        }
