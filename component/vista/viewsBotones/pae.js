...
function pae() {
    const datosGuardados = localStorage.getItem('paeEditados');
    ...
    if (usuario && (usuario.rol === 'editor' || usuario.rol === 'admin')) {
        ...
        function guardarCambios() {
            ...
            localStorage.setItem('paeEditados', JSON.stringify(nuevosDatos));
            return nuevosDatos;
        }
        ...
        if (usuario.rol === 'admin') {
            ...
            botonAgregar.addEventListener('click', () => {
                ...
                [1, "Nuevo Producto", "/", "PAE", ""].forEach(text => {
                    const td = document.createElement('td');
                    td.textContent = text;
            ...
            botonRestaurar.addEventListener('click', () => {
                if (confirm('¿Restaurar datos originales?')) {
                    localStorage.removeItem('paeEditados');
                    document.getElementById('root').innerHTML = "";
                    document.getElementById('root').appendChild(pae());
                }
            });
        }
