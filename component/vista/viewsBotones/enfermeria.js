...
function enfermeria() {
    const datosGuardados = localStorage.getItem('enfermeriaEditados');
    ...
    if (usuario && (usuario.rol === 'editor' || usuario.rol === 'admin')) {
        ...
        function guardarCambios() {
            ...
            localStorage.setItem('enfermeriaEditados', JSON.stringify(nuevosDatos));
            return nuevosDatos;
        }
        ...
        if (usuario.rol === 'admin') {
            ...
            botonAgregar.addEventListener('click', () => {
                ...
                [1, "Nuevo Producto", "/", "ENFERMERÍA", ""].forEach(text => {
                    const td = document.createElement('td');
                    td.textContent = text;
            ...
            botonRestaurar.addEventListener('click', () => {
                if (confirm('¿Restaurar datos originales?')) {
                    localStorage.removeItem('enfermeriaEditados');
                    document.getElementById('root').innerHTML = "";
                    document.getElementById('root').appendChild(enfermeria());
                }
            });
        }
