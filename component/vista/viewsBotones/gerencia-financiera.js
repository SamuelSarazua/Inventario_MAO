...
function gerenFinan() {
    const datosGuardados = localStorage.getItem('gerenciaFinancieraEditados');
    ...
    if (usuario && (usuario.rol === 'editor' || usuario.rol === 'admin')) {
        ...
        function guardarCambios() {
            ...
            localStorage.setItem('gerenciaFinancieraEditados', JSON.stringify(nuevosDatos));
            return nuevosDatos;
        }
        ...
        if (usuario.rol === 'admin') {
            ...
            botonAgregar.addEventListener('click', () => {
                ...
                [1, "Nuevo Producto", "/", "GERENCIA FINANCIERA", ""].forEach(text => {
                    const td = document.createElement('td');
                    td.textContent = text;
            ...
            botonRestaurar.addEventListener('click', () => {
                if (confirm('¿Restaurar datos originales?')) {
                    localStorage.removeItem('gerenciaFinancieraEditados');
                    document.getElementById('root').innerHTML = "";
                    document.getElementById('root').appendChild(gerenFinan());
                }
            });
        }
