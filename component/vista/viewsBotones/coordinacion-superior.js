...
function coordiSuperior() {
    const datosGuardados = localStorage.getItem('coordinacionSuperiorEditados');
    ...
    if (usuario && (usuario.rol === 'editor' || usuario.rol === 'admin')) {
        ...
        function guardarCambios() {
            ...
            localStorage.setItem('coordinacionSuperiorEditados', JSON.stringify(nuevosDatos));
            return nuevosDatos;
        }
        ...
        if (usuario.rol === 'admin') {
            ...
            botonAgregar.addEventListener('click', () => {
                ...
                [1, "Nuevo Producto", "/", "COORDINACIÓN SUPERIOR", ""].forEach(text => {
                    const td = document.createElement('td');
                    td.textContent = text;
            ...
            botonRestaurar.addEventListener('click', () => {
                if (confirm('¿Restaurar datos originales?')) {
                    localStorage.removeItem('coordinacionSuperiorEditados');
                    document.getElementById('root').innerHTML = "";
                    document.getElementById('root').appendChild(coordiSuperior());
                }
            });
        }
